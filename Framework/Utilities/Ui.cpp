...
void UI::Init(Vector3 position, Vector3 size, float rotation, Color color, FillType type)
{
	...
	vs = new VertexShader();
	vs->Create(ShaderPath + L"ColorUI.hlsl", "VS");

	il = new InputLayout();
	il->Create(VertexColor::descs, VertexColor::count, vs->GetBlob());

	ps = new PixelShader();
	ps->Create(ShaderPath + L"ColorUI.hlsl", "PS");
...
void UI::Init(Vector3 position, Vector3 size, float rotation, wstring path, FillType type)
{
	...
	vs = new VertexShader();
	vs->Create(ShaderPath + L"TextureUI.hlsl", "VS");

	il = new InputLayout();
	il->Create(VertexTexture::descs, VertexTexture::count, vs->GetBlob());

	ps = new PixelShader();
	ps->Create(ShaderPath + L"TextureUI.hlsl", "PS");
